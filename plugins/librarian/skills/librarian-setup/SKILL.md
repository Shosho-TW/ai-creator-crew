...
工作區裡有掛載的筆記資料夾 → 確認用哪個（只有一個就不用問），然後**只問一題**：**「空盒子，還是含示範內容？」**——示範內容＝6 張互連的概念卡＋1 張範例卡＋圖譜配色，第一次開 Obsidian 就看得到一張連好的小網（預設含）。
其他一律走預設、不問：資料夾用下表的預設命名、寫作草稿區一併建立（空資料夾無害）。使用者若主動要求自訂名稱或位置，再依要求調整。沒有掛載資料夾 → 請使用者先把筆記資料夾加進工作區，停。
