...
你的圖書館員出門採集。設計哲學：**輕資產、來源誠實**——業界的社群監聽靠重資產爬蟲或商業授權（Meta 關掉 CrowdTangle、Reddit API 收費、Threads API 要商業驗證），一個 skill 複製不了也不假裝能。scout 走 RSS 與開放 API：拿得到的拿好，拿不到的明講。
...
**3. 自訂來源探測.** 使用者補的每個來源，先猜常見 RSS pattern 再**實際抓取驗證**：一般網站 `/feed`、`/rss`、`/index.xml`；Substack `＋/feed`；YouTube `youtube.com/feeds/videos.xml?channel_id=…`（不穩時換 `openrss.org/feeds/youtube/…`）；Podcast 用節目 RSS。驗證的實務陷阱（XML 被判二進位的 workaround、Reddit 封鎖、PubMed 個人 feed 機制、已知無 RSS 名單）見 [`references/fetch-notes.md`](references/fetch-notes.md)。
...
使用者說「每天早上跑」→ 建排程任務掛 Run mode。這就是「每日 PubMed＋部落格掃描」工作流的通用版。
